...
int main()
{
    long long G_S_Iteration(vector<vector<double>> & src, vector<double> & ans, long long max_times, double epsilon);
    void display_equation(vector<vector<double>> & src, vector<double> & ans);
    void display_ans(vector<double> & ans);
    SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), FOREGROUND_INTENSITY);
    cout << "# G_S Iteration." << endl;
    ...
    SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), FOREGROUND_BLUE | FOREGROUND_GREEN | FOREGROUND_RED);
    cout << endl;
    long long times = G_S_Iteration(sample, ans, maximum_iteration_times, epsilon);
    if (0 <= times)
    {
...
long long G_S_Iteration(vector<vector<double>> &src, vector<double> &ans, long long max_times, double epsilon)
{
    ...
    if (ipsilon_reached)
    {
        cout << "\nThe precision is reached, G_S computing terminated." << endl;
    }
    else
    {
        cout << "\nThe precision is not reached and the maximum_iteration_times is reached, G_S iteration terminated." << endl;
    }
    return times;
}